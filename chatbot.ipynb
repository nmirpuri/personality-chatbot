...
def generate_response(prompt):
    ...
    # Remove the prompt from the response if it's repeated
        ...
        return response
def final_response(personality):

  if "hadow" in personailty:
    print ("Enter prompt below") 
    user_input = input()
    output = user_input
    return output
  if "imid" in personality:
    print ("Enter prompt below") 
    user_input = input()
    output = ""
    return output
  if "iscombobulated" in personality: 
    print ("Enter prompt below") 
    user_input = input()
    response = generate_response(user_input)
    output = response
    return output
  if "istracted" in personality:
    print ("Enter prompt below") 
    user_input = input()
    output = "Did you know penguins can't fly? Oh, wait, what were you saying?"
    return output
  else: 
    output = "Please choose one of the above"
  return output
print("Choose between Shadow, Distracted, Timid, or Discombobulated for your chatbot's personality type")
# Example interaction
personailty= input("")
answer = final_response(personailty)
